...
const LuxuryHero = memo(({ onOpenDemo, onOpenDocs }) => {
  ...
  const heroScale = useTransform(smoothProgress, [0, 0.4, 1], [0.6, 0.95, 1]);
  const heroOpacity = useTransform(smoothProgress, [0, 0.2], [0, 1]);
  const contentOpacity = useTransform(smoothProgress, [0, 0.5, 1], [0, 1, 0]);
  const contentY = useTransform(smoothProgress, [0, 0.5, 1], [60, 0, -60]);
  
  const bgImage = "https://upload.wikimedia.org/wikipedia/commons/e/e0/Creaci%C3%B3n_de_Ad%C3%A1n.jpg";
  
  return (
    <section ref={ref} style={{ height: '250vh', position: 'relative' }}>
      <div style={{ position: 'sticky', top: 0, height: '100vh', overflow: 'hidden' }}>
        <motion.div
          style={hasMounted ? { clipPath, scale: heroScale, opacity: heroOpacity } : {}}
          style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}
        >
          <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${bgImage})`, backgroundSize: 'cover', backgroundPosition: 'center', transform: 'scale(1.02)' }} />
          <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at center, ${COLORS.bg}10 0%, ${COLORS.bgDarker} 90%)` }} />
          ...
          <motion.div style={hasMounted ? { opacity: contentOpacity, y: contentY } : {}} style={{ position: 'relative', zIndex: 10, textAlign: 'center', padding: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ ...TRANSITION.medium, delay: 0.1 }}
              style={{ display: 'inline-flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem 1.25rem', background: `${COLORS.accent}15`, borderRadius: '100px', marginBottom: '2rem', border: `1px solid ${COLORS.accent}30`, backdropFilter: 'blur(10px)' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }} style={{ position: 'absolute', bottom: '2rem', left: '50%', transform: 'translateX(-50%)', zIndex: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
